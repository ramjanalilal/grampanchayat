...
frappe.ui.form.on('Receipt', {
	...
	amount_received: function(frm) {
		// frappe.msgprint("can be received = " + String(frm.doc.total_bill_amount - frm.doc.total_received));
		if (frm.doc.amount_received > Math.round(frm.doc.total_bill_amount - frm.doc.total_received)) {
			frm.doc.amount_received = 0
			frm.doc.receipt_tax = []
			frappe.msgprint(__("Amount received cannot be more than pending amount. Please check the pending amount and update amount received!"))
			frm.refresh_fields()
		}
